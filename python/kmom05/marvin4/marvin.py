...
", guessCount, "tries!")
    ...
    if userGuess != number:
        print("Sorry, but the number I was thinking of was " + str(number) + \
"!")
...
def analyzeWords():
    ...
    Analyzes words
    """
    choice = input("Choose a file to analyze. (Or press Enter to continue)\n--\
> ")
    fhand = None
    ...
    try:
        if choice == "":
            ...
            fhand = open("alice-ch1.txt", "r")
        else:
            fhand = open(choice, "r")
    except Exception:
        print("Error opening file:", choice)
    ...
    print("The total amount of words in your desired file was:", wordCount)


    wordFrequency = list()
    for k, v in wordDict.items():
        wordFrequency.append((v, k))

    wordFrequency.sort(reverse=True)

    print("\nThe 7 most frequent words used are (in order of frequency):")
    for k, v in wordFrequency[:7]:
        print(k, v)


    commonFile = open("common-words.txt", "r")
    commonWords = list()
    ...
    frequentUncommons = list()

    for k, v in wordDict.items():
        if k not in commonWords:
            frequentUncommons.append((v, k))

    frequentUncommons.sort(reverse=True)
    print("\nThe 7 most frequent uncommon words used are:")
    for k, v in frequentUncommons[:7]:
        print(k, v)


    spelling = open("words.txt", "r")
    spellCheckList = list()

    for line in spelling:
        line = line.strip()
        line = line.translate(line.maketrans("", "", string.punctuation))
        spellCheckList.append(line)

    spellCheckedWordFrequency = list()
    for k, v in wordDict.items():
        if k in spellCheckList and k not in commonWords:
            spellCheckedWordFrequency.append((v, k))

    spellCheckedWordFrequency.sort(reverse=True)
    print("\nThe 7 most frequent words that are correctly spelled are:")
    for k, v in spellCheckedWordFrequency[:7]:
        print(k, v)


    fhand.close()
    countFile = open(choice, "r")
    wordsList = list()
    ...
    sortedLetterFrequency = list()
    for k, v in letterFrequency.items():
        sortedLetterFrequency.append((v, k))

    sortedLetterFrequency.sort(reverse=True)

    top7Letters = list()

    print(totalLetterCount)

    for count, letter in sortedLetterFrequency[:7]:
        percentage = round((count/totalLetterCount)*100)
        percentage = str(percentage)+"%"
        top7Letters.append(((str(count) + "\t"), (letter + "\t"), percentage))

    print("\nThe 7 most frequent letters, their count, and percentage (of \
total letters)\n\nCount\tLetter\tPercentage")
    for k, v, p in top7Letters:
        print(k, v, p)
